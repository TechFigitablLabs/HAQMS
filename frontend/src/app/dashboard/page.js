...
export default function Dashboard() {
  ...
  // FIX (perf): debounce the patient lookup so we don't hammer the API and re-render
  // the entire parent on every keystroke. We wait 300ms after the user stops typing
  // (or changes gender) before issuing a single fetch. The timeout is cleared on
  // every new keystroke and on unmount.
  useEffect(() => {
            ...
            {selectedPatientHistory && (
              <div className="glass p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-md space-y-4">
                ...
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 text-xs space-y-2">
                  ...
                  {/* FIX: guard against null/undefined medicalHistory.
                      Some patient records (e.g. Bruce Wayne, Clark Kent) have a blank
                      medicalHistory. Calling .toUpperCase() on null previously crashed
                      the entire React tree. We now fall back to a friendly placeholder. */}
