...
export default function QueueMonitor() {
  ...
  useEffect(() => {
    // Track mount state so we don't call setState after the component unmounts
    // (avoids the classic "Can't perform a React state update on an unmounted component" warning).
    let isMounted = true;

    const safeFetchQueueData = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/queue`);
    ...
    // FIX (memory leak): keep a reference to the interval and clear it on unmount.
    // Previously the interval kept firing forever, leaking timers and triggering
    // setState on an unmounted component every 3s for every remount.
    const intervalId = setInterval(() => {
      safeFetchQueueData();
      if (isMounted) setRefreshCount((prev) => prev + 1);
    }, 3000);
