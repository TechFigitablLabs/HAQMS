...
export default function QueueMonitor() {
  ...
  useEffect(() => {
    ...
    // FIX: Store interval ID and return cleanup function to clear it on unmount
    // Without this, every mount creates a new interval that never stops — memory leak
    const intervalId = setInterval(() => {
      fetchQueueData();
      setRefreshCount((prev) => prev + 1);
    }, POLL_INTERVAL_MS);

    return () => clearInterval(intervalId); // <-- the fix
  }, [fetchQueueData]);
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  ...
                  <div>
                    ...
                    {docInfo.waiting.length > 0 ? (
                      ...
                      <span className="text-xs text-slate-400 dark:text-slate-500 italic block">No upcoming patients in queue</span>
                    )}
                  </div>
                </div>
              </div>
