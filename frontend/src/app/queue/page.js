...
export default function QueueMonitor() {
  ...
  useEffect(() => {
    ...
    // FIX: Memory leak — the original code started a setInterval but never returned
    // a cleanup function, so every mount left a dangling interval running forever.
    // Each mount now creates exactly one interval and cancels it on unmount.
    const intervalId = setInterval(() => {
      fetchQueueData();
      setRefreshCount((prev) => prev + 1);
    }, POLL_INTERVAL_MS);

    // Cleanup: cancel the interval when the component unmounts or re-renders
    return () => clearInterval(intervalId);
  }, [fetchQueueData]);

  // Group tokens by doctor for the calling board display
  const groupedTokens = tokens.reduce((groups, token) => {
    ...
    if (token.status === 'WAITING')  groups[docId].waiting.push(token);
    return groups;
  }, {});
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 sm:p-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-400 font-semibold mt-1">
                Real-time physician calling boards. Auto-syncs every {POLL_INTERVAL_MS / 1000} seconds.
              </p>
            </div>
          </div>
        ...
        {error && (
          <div className="p-4 mb-6 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-500 flex items-center gap-3 text-sm">
            ...
            <div><strong>Sync Error:</strong> {error} — Please verify the backend API server is online.</div>
          </div>
        )}
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div className="mb-6">
                    ...
                    {docInfo.calling ? (
                      ...
                      <div className="bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800/80 p-6 rounded-2xl text-center shadow-inner">
                        <span className="block text-2xl font-extrabold text-slate-400 dark:text-slate-500 tracking-wider italic">Idle</span>
                  ...
                  <div>
                    ...
                    {docInfo.waiting.length > 0 ? (
                      ...
                      <span className="text-xs text-slate-400 dark:text-slate-500 italic block">No upcoming patients</span>
                    )}
                  </div>
                </div>
              </div>
